    ...
    void probe_quadratic(studentType student) { // vector<studentType>& hashSet, , const int table_size
        ...
        if (student_quadratic.at(hash_idx).isempty == true){ // if the place is empty
            ...
            if (hash_idx == hashvalue){
                cout << student.sid << " Fail put in.\n" ;
                fail_create_hash = true;
            }
...
public:
    Quadratic() : table_size(0), data_size(0), unsuccessful(0.0), successful(0.0), fail_create_hash(false) {}
    ...
    bool fail_at_create(){
        // cout << "fail_create_hash: " << fail_create_hash << endl;
        return fail_create_hash;
    }

    bool probe(string id){
        bool success = false;
        int probing_times = 0;
        
        int hashvalue = Function(id, table_size); // by hash function to get the hashvalue
        int index = hashvalue;
        // **********************************************************************************
        do{
            probing_times++;
            if(student_quadratic.at(index).isempty){
                success = false;
                break; // empty leave
            }
            else if(string(student_quadratic.at(index).sid) == id){
                success = true;
                // probing_times = 1;
                break; // find leave
            }
            else{
                index = (hashvalue + (probing_times % table_size) * (probing_times % table_size));
                index = index % table_size;
            }
        }
        while(!success && index != hashvalue);
...
public:
    doubleHash() : table_size(0), data_size(0), highest_step(0), unsuccessful(0.0), successful(0.0), fail_create_hash(false) {}
    ...
    bool probe(string id){
        ...
        int hashvalue = Function(id, table_size); // by hash function to get the hashvalue
        int step = step_func(id);
        int index = hashvalue;

        // **********************************************************************************
        if (student_double.at(index).isempty == false && string(student_double.at(index).sid) == id){ // if the place is not empty
            success = true;
            // probing_times = 1;
        }
        else{
            do{
                ...
                if(student_double.at(index).isempty)
                    break;
                if(string(student_double.at(index).sid) == id)
                    success = true;
...
public:
    ...
    void build_quadratic() {
        ...
        if(!q_hashTable.fail_at_create()){ // if the hash table is created successfully
            q_hashTable.calculation(fileID);
            probe_quadratic();
        }
        cout << endl;
    ...
    void build_double() {
        ...
        if(!d_hashTable.fail_at_create()){ // if the hash table is created successfully
            d_hashTable.calculation(fileID);
            probe_double();
        }
        cout << endl;
