...
class Two_Three_Tree{
    ...
    Node *insertdata; // point to the new add data place
    //Node *topPtr;
...
public:
    Two_Three_Tree() : size(0), root(nullptr), insertdata(nullptr){} // constructor of an empty tree
    ...
    void Insert(schoolType pSet, Node *r){
        ...
        if(r == nullptr){
            ...
            if(r->keys.size() == 1){
                if(pSet.nstud == r->keys.at(0))
                    ...
                    Insert(pSet, r->children.at(0));
                else                                // big than key[0], check the right child
                    Insert(pSet, r->children.at(1));
            }
                ...
                if (it != r->keys.end()){ // have the same key
                    ...
                    Insert(pSet, r->children.at(0));
                else if(pSet.nstud < r->keys.at(1)) // small than key[0], check the mid child
                    Insert(pSet, r->children.at(1));
                else                                // big than key[0] & key[1], check the right child
                    Insert(pSet, r->children.at(2));
