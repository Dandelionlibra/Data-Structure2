...
class Two_Three_Tree{
    ...
    void checkcase(Node *&r, int index, int value){
        if(r->isLeaf){
            cout << "\033[1;32mCurrent size is" << r->keys.size() <<" \033[0m" << endl; // green
            if(r->keys.size() > 1){ // delete the value, and done!
                for(int i = 0 ; i < r->keys.size() ; i++)
                    cout << r->keys.at(i) << " ";
                cout << "\n\033[1;32m1.Erase " << r->keys.at(index) <<" \033[0m" << endl; // green
    ...
    Node* Delete(Node *r, int value){
        ...
        vector<int>::iterator it = find(r->keys.begin(), r->keys.end(), value);
        if (it != r->keys.end()){
            cout << "\033[1;35m Find delete position. \033[0m" << endl; // purple
            int index = distance(r->keys.begin(), it);
            
            checkcase(r, index, value);
        ...
        //cannot find same key
            ...
            if(value < r->keys.at(0))
                r->children.at(0) = Delete(r->children.at(0), value);
            else
                r->children.at(1) = Delete(r->children.at(1), value);

        }
        else{ // if(r->keys.size() == 2) // 3 node
            cout << "\033[1;31m Delete: keys.size() == 2 \033[0m" << endl; // red
            if(value < r->keys.at(0)) // small than key[0], check the left child
                r->children.at(0) = Delete(r->children.at(0), value);
            else if(value < r->keys.at(1)) // small than key[1], check the mid child
                r->children.at(1) = Delete(r->children.at(1), value);
            else                                // big than key[0] & key[1], check the right child
                r->children.at(2) = Delete(r->children.at(2), value);
        }
...
public:
    ...
    void outerDelete(int value){
        if(root != nullptr)
            ...
            cout << "\033[1;33m ROOT is null. \033[0m" << endl; // yellow
    ...
    void showheight(){
       ...
       cout << "Number of nodes = " << Nodenum(root) << "\n";
