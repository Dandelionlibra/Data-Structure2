...
class Two_Three_Tree{
    ...
    void checkcase(Node *&r, int index, int key){
        ...
        if(r->isLeaf){
            cout << "\033[1;32mLeaf   Current size is " << r->keys.size() <<" \033[0m" << endl; // green
            if(r->keys.size() == 2){ // delete the value, and done!
                ...
                if(tmp->keys.size() == 1){
                    ...
                    // int tmpchild;
                    // if(r == tmp->children.at(0))  tmpchild = 1;
                    // else if(r == tmp->children.at(1))  tmpchild = 2;
                    // else if(r == tmp->children.at(2))  tmpchild = 1;
                    ...
                    int tkey;
                    vector<int> tno;
                    // tmp and r disconnect with each other
                    r->parent = nullptr; 
                    if(r == tmp->children.at(0)){
                        tmp->children.at(0) = tmp->children.at(1);
                        tmp->children.at(1) = tmp->children.at(2);
                        tmp->children.at(2) = nullptr;

                        // setting the data need rotate down
                        tkey = tmp->keys.at(0);
                        tmp->keys.erase(tmp->keys.begin(), tmp->keys.begin()+1);
                    ...
                    else if(r == tmp->children.at(2)){
                        tmp->children.at(2) = nullptr;

                        // setting the data need rotate down
                        tkey = tmp->keys.at(1);
                        //tmp->keys.clear();
                        //tmp->keys.push_back(tkey);
                        tmp->keys.erase(tmp->keys.begin()+1, tmp->keys.end());

                        tno = tmp->no.at(1);
                        //tmp->no.clear();
                        //tmp->no.push_back(tno);
                        tmp->no.erase(tmp->no.begin()+1, tmp->no.end());
    ...
    void showAll(Node *r){
        ...
        if(r!=nullptr){
            ...
            for(int i = 0 ; i < r->children.size() ; i++){
                cout << "At key " << r->keys.at(0) << "'s " << i << endl;
