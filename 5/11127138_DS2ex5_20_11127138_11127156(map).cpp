...
private:
    ...
    void searchPrimy ( float threshold, const string& orderFile ) {
        ...
        SearchTime = chrono::duration_cast<chrono::microseconds>(end - start);

        writeFile(searchMap, threshold, records );

    }

    void writeFile(const map<int, Graph>& searchMap, float threshold, int records) {
...
int main(){
    ...
    while (continueFlag) {
        ...
        if(fileExistFlag){
            ...
            float threshold = 0;
            // cout << endl << endl << "Please input a threshold in the range [0,1]: " << endl;;
            // cin >>  threshold;
            while ( continueFlag ) {
