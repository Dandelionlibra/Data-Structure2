...
private:
    ...
    void writeBlock(const Graph block[], const string& filename, int size) {
        ofstream outFile(filename, ios::out | ios::binary);
        for(int i = 0 ; i < size ; i++){
            outFile.write(reinterpret_cast<const char*>(&block[i]), sizeof(Graph));
        }
    ...
    void split(const string& fileName) {
        ...
        while (inFile.read(reinterpret_cast<char*>(&oneRecord), sizeof(Graph))) {
            ...
            if (recordIndex == bufferSize) {
                ...
                writeBlock(buffer, blockFileName, recordIndex);
